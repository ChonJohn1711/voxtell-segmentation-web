...
function App() {
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (sessionId) {
      fetch(`http://localhost:1711/session/${sessionId}`, { method: 'DELETE' }).catch(() => {});
      setSessionId(null);
    }
    ...
    if (file.name.toLowerCase().endsWith('.zip')) {
      ...
      try {
        ...
        const response = await fetch('http://localhost:1711/convert', {
          method: 'POST',
          body: formData,
        });
  ...
  const handleSegmentation = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:1711/predict', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleExportRtstruct = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:1711/export-rtstruct', {
        method: 'POST',
        body: formData,
      });
