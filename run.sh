...
#echo -e "${CYAN}Starting backend...${RESET}"
#echo -e "  ${YELLOW}CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-not_set}${RESET}"
#echo -e "  ${YELLOW}Note:${RESET} Model loading takes 30–60 s — inference won't work until you see 'Model loaded successfully.'"
#
#conda run --no-capture-output -n voxtell python -c "import os, torch; print('CUDA_VISIBLE_DEVICES=', os.environ.get('CUDA_VISIBLE_DEVICES')); print('device_count=', torch.cuda.device_count()); print('device_name_0=', torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'CPU')"
#
#conda run --no-capture-output -n voxtell python "$SCRIPT_DIR/backend/server.py" &
#BACKEND_PID=$!
...
(cd "$SCRIPT_DIR/frontend" && npm run dev -- --host 0.0.0.0 --port 2811 --strictPort 2>&1) &
FRONTEND_PID=$!

echo ""
echo -e "  Backend:  ${GREEN}http://localhost:1711${RESET}"
echo -e "  Frontend: ${GREEN}http://localhost:2811${RESET}  ${BOLD}← open this${RESET}"
echo ""
